...
class Repair extends CI_Controller {
    ...
    public function getRepairList(){
        //$conditions = "order by repairId desc limit 15";
        //$conditions = "ORDER BY repairId DESC";
        //$repairList = DB::select('repair',['*']);
        //$repairList = DB::select('repair',['*'],$conditions);
        $sql = 'select * FROM repair WHERE onoff="on" order by repairId desc ';
        $repairList  = (DB::raw($sql))->fetchAll(PDO::FETCH_ASSOC);
        $this->json($repairList);
    }
    ...
    public function userPublishRepair(){
        ...
        $repair['onoff'] = 'on';
        $repair['pubDate'] =  date("Y-m-d H:i:s",time());
        if($ismingan['errcode'] == 87014){//是否通过敏感测试
            $this->json(['mingan'=>true]);//敏感
        }else{
            DB::insert('repair',$repair);
            $repairId = (DB::row('repair',['repairId'],['content'=>$repair['content'],'pubDate'=>$repair['pubDate']]))->repairId;
